...
function t:IsArray(o)local n=0
...
self:Append(o)end
function t:WriteError(n)o(e.format("Encoding of %s unsupported",c(n)))end
...
i(self:Next()=='"')local t=function(n)return e.char(u(n,16))end
...
local e=print
local e=coroutine
module(...)function request(f,c,l,e)local n=n.newhandler("coroutine")local o=true
n:start(function()local r,s=i.sink.table()local a,t
if e then
if e.headers then
a=e.headers
end
if e.body then
t=i.source.string(e.body)end
end
local n,t,e=d.request{url=f,method=c,create=n.tcp,sink=r,source=t,headers=a}if n then
l{statusCode=t,headers=e,response=u.concat(s),sink=r,isError=false}else
l{isError=true}end
o=false
end)local e={}function e.enterFrame()if o then
n:step()else
...
end,remove=function(r,n)local i=e[n]if i then
e[n]=nil
local t=o.remove(r)if t~=n then
e[t]=i
r[i]=t
end
end
...
function i:send(a,n,l)n=(n or 1)-1
local r,i
while true do
if t.yield(o.sending,e)=="timeout"then
return nil,"timeout"end
r,i,n=e:send(a,n+1,l)if i~="timeout"then return r,i,n end
end
end
function i:receive(a,n)local i="timeout"local l
while true do
if t.yield(o.receiving,e)=="timeout"then
return nil,"timeout"end
l,i,n=e:receive(a,n)if(i~="timeout")or r then
return l,i,n
end
end
end
...
function abort(o,e)local n=o.cortn[e]if n then
kick(o,e)t.resume(n,"timeout")end
end
...
return o.encode({device=n.device,sdk={name=REVMOB_SDK["NAME"],version=REVMOB_SDK["VERSION"]}})end,post=function(o,t,n)if t==nil then return end
...
end,postWithoutFollowRedirect=function(o,e,t)if e==nil then return end
if not t then t=function(e)end
end
local n={}n.body=e
n.headers={["Content-Length"]=tostring(#e),["Content-Type"]="application/json"}asyncHttp.request(o,"POST",t,n)end,fetch=function(e,n)if RevMobSessionManager.isSessionStarted()then
...
end,install=function(e,n)RevMobClient.post(e:urlInstall(),e:payloadAsJsonString(),n)end,startSession=function(e)RevMobClient.post(e:urlSession(),e:payloadAsJsonString(),listener)end,theFetchSucceed=function(r,i,t)local e=i.status or i.statusCode
...
return n,e
end,getMarketURL=function(n,e)local i=require('socket.http')local t=require("ltn12")local o={}if e==nil then
e=""end
local t,e,o=i.request{method="POST",url=n,source=t.source.string(e),headers={["Content-Length"]=tostring(#e),["Content-Type"]="application/json"},sink=t.sink.table(o),}if(e==302 or e==303)then
...
local n=RevMobClient:new("fullscreens",o)n:fetch(e._networkListener)end,loadImage=function(e)if e._released==true then log("Fullscreen was closed.")return end
...
display.loadRemoteImage(e.imageUrl,"GET",e._loadImageListener,"fullscreen.jpg",system.TemporaryDirectory)end,loadCloseButton=function(e)if e._released==true then return end
...
end,load=function(e,i)e.networkListener=function(t)local o,t=RevMobClient.theFetchSucceed(n,t,e.listener)if o then
...
end)package.preload['fullscreen_chooser']=(function(...)require('revmob_messages')require('revmob_client')require('revmob_utils')require('fullscreen')require('fullscreen_web')local o="fullscreen"FullscreenChooser={listener=nil,placementID=nil,fullscreen=nil,autoshow=true,new=function(e)local e=e or{}setmetatable(e,FullscreenChooser)return e
end,load=function(e)local n=function(n)local t,n=RevMobClient.theFetchSucceed(o,n,e.listener)if t then
local n=n['fullscreen']['links']local t=RevMobUtils.getLink('clicks',n)local i=RevMobUtils.getLink('html',n)local r=RevMobUtils.getLink('image',n)local n=RevMobUtils.getLink('close_button',n)if e.listener~=nil then e.listener({type=REVMOB_EVENT_AD_RECEIVED,ad=o})end
if i then
log("Rich fullscreen")e.fullscreen=FullscreenWeb.new({listener=e.listener})e.fullscreen.htmlUrl=i
e.fullscreen.clickUrl=t
...
end)package.preload['banner']=(function(...)require('revmob_messages')require('revmob_client')require('revmob_utils')local t="banner"Banner={autoshow=true,listener=nil,clickUrl=nil,imageUrl=nil,component=nil,_clicked=false,_released=false,width=nil,height=nil,x=nil,y=nil,new=function(e)local e=e or{}setmetatable(e,Banner)e.component=display.newGroup()e.component.alpha=0
return e
end,load=function(e,o)e.networkListener=function(n)local n,t=RevMobClient.theFetchSucceed(t,n,e.listener)if n then
local n=t['banners'][1]['links']e.clickUrl=RevMobUtils.getLink('clicks',n)e.imageUrl=RevMobUtils.getLink('image',n)e:loadImage()end
end
local n=RevMobClient:new("banners",o)n:fetch(e.networkListener)end,loadImage=function(e)if e._released==true then log("Banner was released.")return end
e._loadImageListener=function(n)if e._released==true then if n.target then n.target:removeSelf()end log("Banner was released.")return end
if n.isError or n.target==nil or e.imageUrl==nil then
log("Fail to load ad image: "..tostring(e.imageUrl))if e.listener~=nil then e.listener({type=REVMOB_EVENT_AD_NOT_RECEIVED,ad=t})end
return
end
e.image=n.target
local o=(RevMobScreen.width()>640)and 640 or RevMobScreen.width()local n=(RevMobDevice:isIPad()and 100 or 50*(RevMobScreen.bottom()-RevMobScreen.top())/display.contentHeight)local r=(RevMobScreen.left()+o/2)local i=(RevMobScreen.bottom()-n/2)e:setPosition(e.x or r,e.y or i)e:setDimension(e.width or o,e.height or n)e.image.tap=function(n)if not e._clicked then
e._clicked=true
if e.listener~=nil then e.listener({type=REVMOB_EVENT_AD_CLICKED,ad=t})end
local n=RevMobClient.getMarketURL(e.clickUrl)log(REVMOB_MSG_OPEN_MARKET)if n then system.openURL(n)end
e:release()end
return true
end
e.image.touch=function(n)return true end
e.image:addEventListener("tap",e.image)e.image:addEventListener("touch",e.image)e.component:insert(1,e.image)if e.listener~=nil then e.listener({type=REVMOB_EVENT_AD_RECEIVED,ad=t})end
if e.autoshow then
e:show()end
end
...
if e.component~=nil then
e.component.alpha=1
if e.listener~=nil then e.listener({type=REVMOB_EVENT_AD_DISPLAYED,ad=t})end
end
end,setPosition=function(e,n,t)if e.image~=nil then
e.image.x=n or e.image.x
e.image.y=t or e.image.y
end
end,setDimension=function(e,t,n,o)if e.image~=nil then
...
end,setPosition=function(n,t,e)if n.webView then
if t~=0 or e~=0 then
log(o)end
end
end,setDimension=function(e,o,t,n)if e.webView then
e.webView.width=o or e.webView.width
e.webView.height=t or e.webView.height
e.webView.rotation=n or e.webView.rotation
end
end,update=function(e,n,t,i,o,r)e:setPosition(n,t)e:setDimension(i,o,r)end,release=function(e)if e.webView then
