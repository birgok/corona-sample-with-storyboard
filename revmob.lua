...
local n=true;for t,e in a(e)do
if not n then
self:Append(',')end
...
r(self:Next()=='"')local t=function(n)return e.char(u(n,16))end
...
end)package.preload['asyncHttp']=(function(...)local e=require"socket"local n=require"dispatch"local d=require"socket.http"local s=require"ltn12"n.TIMEOUT=10
local t=Runtime
local f=table
local e=print
local e=coroutine
module(...)function request(u,c,i,e)local n=n.newhandler("coroutine")local l=true
n:start(function()local t,a=s.sink.table()local r,o
if e then
if e.headers then
r=e.headers
end
if e.body then
o=s.source.string(e.body)end
end
local e,n,r=d.request{url=u,method=c,create=n.tcp,sink=t,source=o,headers=r}if e then
i{statusCode=n,headers=r,response=f.concat(a),sink=t,isError=false}else
i{isError=true}end
l=false
...
end,remove=function(l,r)local n=e[r]if n then
e[r]=nil
local t=i.remove(l)if t~=r then
e[t]=n
l[n]=t
end
end
...
e:settimeout(0)local c={__index=function(i,t)i[t]=function(...)arg[1]=e
...
end,load=function(e,r)e.networkListener=function(t)local i,t=Client.theFetchSucceed(n,t,e.listener)native.setActivityIndicator(false)if i then
...
end)package.preload['fullscreen_chooser']=(function(...)require('revmob_messages')require('revmob_client')require('revmob_utils')require('fullscreen')require('fullscreen_web')local r="fullscreen"FullscreenChooser={show=function(e,l)networkListener=function(n)local t,n=Client.theFetchSucceed(r,n,e)native.setActivityIndicator(false)if t then
local n=n['fullscreen']['links']local i=getLink('clicks',n)local t=getLink('html',n)local l=getLink('image',n)local n=getLink('close_button',n)if e~=nil then e({type=REVMOB_EVENT_AD_RECEIVED,ad=r})end
if t then
local e=FullscreenWeb.new({listener=e})e.htmlUrl=t
e.clickUrl=i
e:show()else
Fullscreen.adListener=e
Fullscreen.clickUrl=i
...
return e
end,load=function(e,r)e.networkListener=function(t)local t,i=Client.theFetchSucceed(n,t,e.listener)if t then
...
end,update=function(e,o,l,t,r,n)if e.webView then
if o~=0 or l~=0 then
log(i)end
...
local e=Client:new("links",r)e.post(e:url(),e:payloadAsJsonString(),internalListener)else
...
client=Client:new("pop_ups",n)client:fetch(Popup.networkListener)end,networkListener=function(e)local t,e=Client.theFetchSucceed(n,e,Popup.adListener)if t then
...
native.setActivityIndicator(true)showFullscreenInTheNextFrame=function()Runtime:removeEventListener("enterFrame",showFullscreenInTheNextFrame)local e=RevMob.getRevMobIDAccordingToPlatform(e)FullscreenChooser.show(n,e)end,timer.performWithDelay(t,function()native.setActivityIndicator(false)end)Runtime:addEventListener("enterFrame",showFullscreenInTheNextFrame)end,showFullscreenWeb=function(n,e)if not RevMobSessionManager.isSessionStarted()then return log(REVMOB_MSG_NO_SESSION)end
native.setActivityIndicator(true)local n=FullscreenWeb.new(n)showFullscreenWebInTheNextFrame=function()Runtime:removeEventListener("enterFrame",showFullscreenWebInTheNextFrame)local e=RevMob.getRevMobIDAccordingToPlatform(e)n:load(e)end,timer.performWithDelay(t,function()native.setActivityIndicator(false)end)Runtime:addEventListener("enterFrame",showFullscreenWebInTheNextFrame)end,showFullscreenImage=function(e,n)native.setActivityIndicator(true)showFullscreenImageInTheNextFrame=function()Runtime:removeEventListener("enterFrame",showFullscreenImageInTheNextFrame)local n=RevMob.getRevMobIDAccordingToPlatform(n)Fullscreen.show(e,n)end,timer.performWithDelay(t,function()native.setActivityIndicator(false)end)Runtime:addEventListener("enterFrame",showFullscreenImageInTheNextFrame)end,openAdLink=function(n,e)if not RevMobSessionManager.isSessionStarted()then return log(REVMOB_MSG_NO_SESSION)end
...
local e=RevMob.getRevMobIDAccordingToPlatform(e)Popup.show(n,e)end,printEnvironmentInformation=function(e)log("==============================================")log("RevMob Corona SDK: "..REVMOB_SDK["NAME"].." - "..REVMOB_SDK["VERSION"])log("App ID in session: "..tostring(RevMobSessionManager.appID))if e then
